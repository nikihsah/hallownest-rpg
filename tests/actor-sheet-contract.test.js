...
test("quick trait attacks are exposed through the selected-token HUD", async () => {
  ...
  assert.match(hud, /height: Math\.min\(Math\.floor\(viewportHeight \* 0\.86\), 780\)/);
...
test("long item and catalog descriptions scroll inside their panels", async () => {
  ...
  assert.match(styles, /\.application:has\(\.hrpg-attack-dialog\)[^}]*max-height: 86vh/s);
  ...
  assert.match(styles, /\.hrpg-attack-dialog \{[^}]*height: 100%/s);
  assert.match(styles, /\.hrpg-attack-dialog \{[^}]*max-height: calc\(86vh - 7rem\)/s);
  ...
  assert.match(styles, /\.hrpg-defense-dialog \{[^}]*height: 100%/s);
  assert.match(styles, /\.hrpg-defense-dialog \{[^}]*max-height: calc\(86vh - 7rem\)/s);
