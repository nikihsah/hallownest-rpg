...
test("long item and catalog descriptions scroll inside their panels", async () => {
  ...
  assert.match(styles, /\.application\.dialog:has\(\.hrpg-attack-dialog\)[^}]*max-height: 86vh/s);
  assert.match(styles, /\.application\.dialog:has\(\.hrpg-attack-dialog\) \.window-content[^}]*overflow: hidden/s);
  assert.match(styles, /\.application\.dialog:has\(\.hrpg-attack-dialog\) \.dialog-buttons[^}]*flex: 0 0 auto/s);
  assert.match(styles, /\.hrpg-attack-dialog \{[^}]*max-height: min\(65vh, 42rem\)/s);
  ...
  assert.match(styles, /\.hrpg-defense-dialog \{[^}]*max-height: min\(65vh, 42rem\)/s);
