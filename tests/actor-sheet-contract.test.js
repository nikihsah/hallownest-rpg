...
test("quick trait attacks are exposed through the selected-token HUD", async () => {
  ...
  assert.match(hud, /SCROLL_DIALOG_BODY_STYLE/);
  ...
  assert.equal((hud.match(/class="hrpg-dialog-scroll"/g) ?? []).length, 5);
  assert.match(hud, /style="\$\{SCROLL_DIALOG_BODY_STYLE\}"/);
...
test("long item and catalog descriptions scroll inside their panels", async () => {
  ...
  assert.match(styles, /\.hrpg-attack-dialog \{[^}]*overflow: visible !important/s);
  assert.match(styles, /\.hrpg-defense-dialog \{[^}]*overflow: visible !important/s);
  assert.match(styles, /\.hrpg-dialog-scroll \{[^}]*height: min\(32rem, calc\(100vh - 12rem\)\)/s);
  assert.match(styles, /\.hrpg-dialog-scroll \{[^}]*max-height: min\(32rem, calc\(100vh - 12rem\)\)/s);
  assert.match(styles, /\.hrpg-dialog-scroll \{[^}]*overflow-y: scroll !important/s);
  assert.match(styles, /\.hrpg-dialog-scroll \{[^}]*overscroll-behavior: contain/s);
