...
export async function rollDicePool({ dice, label = "Проверка", actor = null } = {}) {
  ...
  await roll.toMessage({
    speaker,
    flavor: `<strong>${foundry.utils.escapeHTML(label)}</strong><br>${game.i18n.format("HRPG.RollResult", { successes })}`
  });
